...
class Calibration(DefaultCalibration):
    def handle_substitutions(self) -> None:
        ...
        super().handle_substitutions({
            'SWEEP_STEP':       f'{sweep_width/(sweep_npoints-1):.4f}',
            'SWEEP_HALF_WIDTH': str(sweep_width/2)
        })

    def pre_process(self) -> None:
        super().pre_process(mapping = {
            'PULSE_LENGTH': str(self.assumptions['spectro_qubit'][f'{self.name}_pulse_length']),
            'PULSE_AMP':    str(self.assumptions['spectro_qubit'][f'{self.name}_pulse_amp'])
        })
